...
void VideoPlayer::loadVideoFiles(const QString &folderPath) {
    ...
    // Use preprocessor directives to differentiate between operating systems
#if defined(Q_OS_WIN)
    // On Windows, load .wmv files
    files = dir.entryInfoList(QStringList() << "*.wmv", QDir::Files);
#elif defined(Q_OS_LINUX) || defined(Q_OS_MAC)
    // On Linux and macOS, load .mp4 and .mov files
    files = dir.entryInfoList(QStringList() << "*.mp4" << "*.mov", QDir::Files);
#endif
...
void VideoPlayer::playVideo(int index) {
    if (index >= 0 && index < videoFiles.size()) {
        ...
        QString videoName = fileInfo.baseName(); // baseName() gives the name without the path and extension

        // Now emit a signal with this video name
        emit videoNameChanged(videoName);
    }
}
...
void VideoPlayer::keyPressEvent(QKeyEvent *event) {
    ...
    switch (event->key()) {
    case Qt::Key_Up:
        qDebug() << "Play previous video";
        playPreviousVideo();
        break;
    case Qt::Key_Down:
        qDebug() << "Play next video";
        playNextVideo();
        break;
    default:
